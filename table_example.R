...
df_nbd_pro <- df_nbd %>%
  tibble::as_tibble() %>%
  janitor::clean_names() %>%
  dplyr::select_at(vars(c(1, paste0(
    "x", as.character(2010:2018)
  )))) %>%
  dplyr::rename_at(vars(dplyr::starts_with("x")), ~ paste0("n", .x)) %>%
  dplyr::rename(nbd_country = country_name) %>%
  dplyr::mutate(
    iso3c = countrycode::countrycode(nbd_country, origin = 'country.name', destination = 'iso3c'),
...
df_vci_pro <- df_vci %>%
  ...
  janitor::clean_names() %>%
  dplyr::select_at(vars(c(1:3, paste0(
    "x", as.character(2010:2018)
  )))) %>%
  ...
  dplyr::mutate(
    iso3c = countrycode::countrycode(vci_country,
                                     origin = 'country.name',
                                     destination = 'iso3c'),
...
df_patents_pro <- df_patents %>%
  ...
  dplyr::mutate(
    iso3c = countrycode::countrycode(patent_office,
                                     origin = 'country.name',
                                     destination = 'iso3c'),
    patents_2010 = px2010,
    patents_2018 = px2018
...
all_data <- countrycode::codelist %>%
  dplyr::filter(
    ...
    include_fa <- function(x, y, rank) {
      if (!is.na(y)) {
        name <-
          ifelse(x > y,
                 "arrow-circle-down",
                 ifelse(x < y, "arrow-circle-up", "arrow-circle-right"))
        color <-
          ifelse(is.na(x) ||
                   is.na(y),
                 "transparent",
                 ifelse(x > y, "#ff7f00", ifelse(x < y, "#00FF7F", "white")))
        ...
        paste0(c(
          as.character(fontawesome::fa(name = name,
                                       fill = color)),
          kableExtra::cell_spec(
            y,
            font_size = 14,
    ...
    europe_mean <- all_data %>%
      ...
      dplyr::select_at(vars(dplyr::starts_with(c(
        "px", "vx", "nx"
      )))) %>%
      ...
      tidyr::nest(
        patent_data = dplyr::starts_with("px"),
    ...
    tbl_data <- tbl_data %>%
      ...
      dplyr::mutate(
        patent_hist = purrr::map_chr(patent_data,
                                     ~ sparkline::spk_chr(as.numeric(.x[1,]), type = 'bar')),
        vci_hist = purrr::map_chr(vci_data,
                                  ~ sparkline::spk_chr(as.numeric(.x[1,]), type = 'bar')),
        nbd_hist = purrr::map_chr(
          new_business_data,
          ~ sparkline::spk_chr(as.numeric(.x[1,]), type = 'bar')
        ),
        Flag = paste0(
          '<img src="https://cdn.countryflags.com/thumbs/',
          stringr::str_to_lower(
            stringr::str_replace_all(
              dplyr::if_else(
                country.name.en == "Czechia",
                "czech-republic",
                country.name.en
              ),
              pattern = " ",
    ...
    tbl_data <- tbl_data %>%
      dplyr::mutate(
        patents_2018 = purrr::pmap_chr(list(patents_2010,
                                            patents_2018,
                                            rank_pat),
                                       include_fa),
        venture_capital_2018 = purrr::pmap_chr(
          list(venture_capital_2010,
               venture_capital_2018,
               rank_vc),
          include_fa
        ),
        ...
        allover_rank = kableExtra::cell_spec(
          allover_rank,
          font_size = 20,
          background = "#eeeeee",
          color = "gray",
